def generate_noisy_trajectory():
    ...
    t_constant_speed = np.linspace(initial_pause + acceleration_time, initial_pause + acceleration_time + constant_speed_time, constant_speed_time + 1)
    t_deceleration = np.linspace(initial_pause + acceleration_time + constant_speed_time, initial_pause + acceleration_time + constant_speed_time + deceleration_time, deceleration_time + 1)
    t_final_pause = np.linspace(initial_pause + acceleration_time + constant_speed_time + deceleration_time, initial_pause + acceleration_time + constant_speed_time + deceleration_time + final_pause, final_pause + 1)
    ...
    # 添加噪声
    noisy_x_total = x_total + np.random.normal(0, noise_sigma, x_total.shape)



    # 生成输出文件
    noisy_trajectory_file_path = "noisy_trajectory_gt.txt"
    with open(noisy_trajectory_file_path, "w") as file:
        for time, x, y in zip(t_total, noisy_x_total, y_total):
